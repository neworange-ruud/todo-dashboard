 ...
 *
 * Verified on this machine (11 Sep 2026):
 *   - `GET  /api/v1/health`         → 200, unauthenticated, per-service status.
 *   - `POST /api/v1/search`         → 401 without a key.
 *   - `GET  /api/v1/documents/{id}` → 401 without a key.
 *
 * **We do not have an Omni API key yet.** `OMNI_API_KEY` is therefore optional and read
 * straight from the environment rather than through `getEnv()` (which fails fast on missing
 * *required* keys). Until the key exists every authenticated call returns
 * `{ ok: false, reason: 'unconfigured' }` and the drill-in renders
...
/**
 * One HTTP round-trip. Throws {@link OmniRequestError}; callers convert to a result.
 *
...
export function toDocument(raw: unknown): OmniDocument | null {
  ...
  return {
    ...
    url: firstString(source.url, metadata.url),
